...
import { ArrowLeftIcon, ArrowBendDownRightIcon, ChatCircleTextIcon, ThumbsUpIcon } from '@phosphor-icons/react';
import { DiscussionThread } from '@/lib/data/community';
import { Link } from '@/i18n/routing';
import { CommunitySidebar } from '@/components/community/CommunitySidebar';

interface TopicViewProps {
  topic: DiscussionThread;
}
    ...
    <div className="min-h-screen bg-bg-base relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[60px_1fr_350px] lg:grid-rows-[48px_1fr] min-h-screen lg:h-screen lg:overflow-hidden max-w-full relative z-10">
        ...
        <main className="p-4 lg:p-8 flex flex-col gap-10 overflow-visible lg:overflow-y-auto w-full">
          {/* Header Actions */}
          <div>
            <Link href="/community" className="inline-flex items-center gap-2 text-[10px] uppercase tracking-widest font-bold text-ink-secondary hover:text-ink-primary transition-colors mb-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-ink-secondary/20 dark:border-border pb-6">
              <div>
                ...
                <div className="text-[11px] uppercase tracking-widest text-ink-secondary mt-4 flex items-center gap-2">
                   <i className={topic.avatar}></i> INIT BY {topic.author} <span className="text-ink-tertiary">|</span> {topic.lastActive}
                </div>
              </div>
          ...
          <div className="flex flex-col gap-8">
            ...
            <div>
              <div className="flex items-center gap-2 mb-6">
                ...
                <h2 className="font-bold uppercase tracking-widest text-[13px]">RESPONSES ({topic.comments.length})</h2>
              </div>
              
              <div className="flex flex-col gap-4">
                {topic.comments.length > 0 ? topic.comments.map(comment => (
                  <div key={comment.id} className="border border-ink-secondary/20 bg-surface/30 p-5 relative group hover:border-ink-primary/50 transition-colors">
                     ...
                     <div className="flex items-center justify-between mb-3 border-b border-dashed border-ink-secondary/20 pb-2">
                        <div className="text-[10px] uppercase tracking-widest font-bold flex items-center gap-1.5 border border-ink-secondary/30 px-1.5 py-0.5 bg-bg-surface">
                           <i className={comment.avatar}></i> {comment.author}
                        </div>
            ...
            <div className="mt-4 border border-ink-primary bg-bg-base relative p-1 pb-1">
               ...
               <div className="flex justify-end p-2 border-t border-ink-primary/20 bg-bg-surface">
                 <Button className="rounded-none bg-ink-primary text-bg-base uppercase text-[10px] tracking-widest font-bold hover:bg-ink-primary/90 px-6">
                    TRANSMIT
                 </Button>
               </div>
            </div>
