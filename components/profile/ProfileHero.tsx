...
export function ProfileHero({ profile }: ProfileHeroProps) {
	return (
		<div className="grid grid-cols-1 md:grid-cols-[180px_1fr] gap-8 p-8 border border-border bg-bg-surface relative">
			...
			{/* Avatar */}
			<div className="w-[180px] h-[180px] border border-ink-primary p-2 bg-bg-surface relative overflow-hidden">
				{profile.avatar ? (
					<Image
						src={profile.avatar}
			...
			<div className="flex flex-col justify-between">
				<div>
					...
					<p className="text-ink-secondary max-w-[500px] mt-3">{profile.bio}</p>
				</div>

				<div>
					<div className="text-[10px] uppercase tracking-widest text-ink-secondary mb-4">
						ENROLLED SINCE: {profile.enrolledSince} {"//"} LOC:{" "}
						{profile.location}
					</div>
